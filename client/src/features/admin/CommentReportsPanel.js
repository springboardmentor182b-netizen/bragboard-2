...
const CommentReportsPanel = () => {
    ...
    const [isDeleting, setIsDeleting] = useState(null); // ID of comment being deleted
    ...
    const loadReports = useCallback(async () => {
        ...
        setError('');
        try {
            const data = await fetchCommentReports({
                adminId,
                status: statusFilter === 'all' ? undefined : statusFilter,
            });
            setReports(Array.isArray(data) ? data : []);
        } catch (err) {
            console.error("fetchCommentReports failed:", err);
            setError(err?.message || 'Unable to load comment reports.');
        } finally {
            setLoading(false);
        }
    ...
    const handleResolveSubmit = async ({ status, resolutionNotes }) => {
        ...
        setError('');
        try {
            const updated = await resolveCommentReport(selectedReport.id, {
                adminId,
                status,
                resolutionNotes,
            });
    ...
    const handleDeleteComment = async (commentId) => {
        ...
        setError('');
        try {
            await deleteComment(commentId, adminId);
            // Refresh reports
            await loadReports();
        } catch (err) {
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between px-6 py-5 border-b border-gray-800">
                <div>
                    <div className="flex items-center gap-3">
                        <h2 className="text-xl font-bold text-white">Comment Reports</h2>
                        {pendingCount > 0 && (
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                        Review reported comments and resolve or dismiss them.
                    </p>
                </div>
            ...
            <div className="px-6 py-4">
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-2 py-16 text-gray-400">
                        ...
                        <p>No comment reports found for this filter.</p>
                    </div>
                ) : (
                        ...
                        <table className="min-w-full text-sm text-gray-200">
                            <thead className="text-xs uppercase text-gray-400">
                                <tr className="border-b border-gray-800">
                                    ...
                                    <th className="text-left py-3 pr-4">Comment</th>
                                ...
                                {reports.map((report) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-3 pr-4 max-w-xs">
                                            <p className="text-gray-100">{report.comment_content || '—'}</p>
                                            {report.comment_author_id && (
                                                <p className="text-xs text-gray-500 mt-1">
                                                    Author ID: {report.comment_author_id}
                                                </p>
                                            )}
                                        </td>
                                        ...
                                        <td className="py-3 pr-4">
                                            {report.status === 'pending' ? (
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    {report.comment_content && report.status === 'pending' && (
                                                        <button
                                                            onClick={() => handleDeleteComment(report.comment_id)}
                                                            disabled={isDeleting === report.comment_id}
                                                        ...
                                                        >
                                                            {isDeleting === report.comment_id ? 'Deleting...' : 'Delete Cmt'}
                                                        </button>
                                                    )}
                                                </div>
                                            ) : (
...
export default CommentReportsPanel;
